...
from bilby.gw.conversion import component_masses_to_chirp_mass, convert_to_lal_binary_neutron_star_parameters, generate_mass_parameters
...
def transformed_samples(samples):
    samples, _ = convert_to_lal_binary_neutron_star_parameters(samples)
...
lal_posterior_samples = {
        list(bilby_to_lalinference.keys())[list(bilby_to_lalinference.values()).index(key)]: lal['IMRPhenomPv2NRT_lowSpin_posterior'][key]
...
lal_prior_samples = {
        list(bilby_to_lalinference.keys())[list(bilby_to_lalinference.values()).index(key)]: lal['IMRPhenomPv2NRT_lowSpin_prior'][key]
        for key in bilby_to_lalinference.values()
}
prior_length = len(lal_prior_samples['mass_1'])

# Set up the bilby prior
bilby_prior_dict = PriorDict(filename=args.prior_path, conversion_function=convert_to_lal_binary_neutron_star_parameters)
